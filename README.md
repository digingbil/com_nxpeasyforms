...
-   **Webhook dispatcher logging**: Added proper error logging to `GenericWebhookDispatcher` and `SlackDispatcher`. Failed webhook deliveries now log warnings with the error message, endpoint (where applicable), and form ID instead of silently swallowing exceptions.
...
-   **Async integration queue**: Documented the PSR-16 backed `IntegrationQueue` that buffers Zapier, Make, Slack, Mailchimp and HubSpot jobs to prevent slow endpoints from blocking submissions while still retrying failed dispatches.
